...
const PhotoOf_EmergencyWork = require('../models/PhotoOf_EmergencyWork');
...
//@desc   Add photo
//@route  POST /api/v1/photo/emergencywork
//@access Private
exports.addPhoto = asyncHandler(async (req, res, next) => {
  ...
  const image = new PhotoOf_EmergencyWork({
...
//@desc   Get all photos
//@route  GET /api/v1/photo/emergencywork
//@access Private
exports.getAllPhotos = asyncHandler(async (req, res, next) => {
  const allPhoto = await PhotoOf_EmergencyWork.find();
  //Check if photo exists
  if (!allPhoto) {
...
//@desc   Get one photo
//@route  GET /api/v1/photo//emergencywork/:id
//@access Private
exports.getOnePhoto = asyncHandler(async (req, res, next) => {
  const onePhoto = await PhotoOf_EmergencyWork.findById(req.params.id);
  //Check if photo exists
  if (!onePhoto) {
...
//@route  DELETE /api/v1/photo//emergencywork/:id
//@access Private
exports.deletePhoto = asyncHandler(async (req, res, next) => {
  const onePhoto = await PhotoOf_EmergencyWork.findByIdAndDelete(req.params.id);
